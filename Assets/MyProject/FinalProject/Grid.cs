...
public class Grid : MonoBehaviour
{
    ...
    private int[] parentNode = new int[CELL_COUNT * CELL_COUNT];
    ...
    void Start()
    {
        ...
        for (int a = 0; a < CELL_COUNT * CELL_COUNT; a++)
        {
            ...
            if (Terrain.activeTerrain.SampleHeight(CellNumToVector3(a)) > 0.1f)
            {
                ...
                isBlocked[a] = true;
                Debug.DrawLine(new Vector3(a % CELL_COUNT, 0.1f, a / CELL_COUNT), new Vector3(a % CELL_COUNT + 1, 0.0f, a / CELL_COUNT + 1), Color.black, Mathf.Infinity);
            }
            else isBlocked[a] = false;
        }
    ...
    //Referenced by professor's code
    List<int> NeighborCell(int cellNum)
    {
        List<int> neighbors = new List<int>() { -1, 1, -CELL_COUNT, CELL_COUNT, -CELL_COUNT - 1, -CELL_COUNT + 1, CELL_COUNT - 1, CELL_COUNT + 1 };
        //string[] str = new string[3];

        if (cellNum % CELL_COUNT == 0) neighbors.RemoveAll((num) => { return num == -1 || num == -1 - CELL_COUNT || num == -1 + CELL_COUNT; });
        ...
        for (int a = 0; a < neighbors.Count;)
        {
            ...
            if (isBlocked[neighbors[a]] == true) neighbors.RemoveAt(a);
            else a++;
